...
interface EmployabilityCrisisProps {
  className?: string;
}

const EmployabilityCrisis: React.FC<EmployabilityCrisisProps> = ({ className = '' }) => {
  ...
  const unemployableCount = totalCandidates - employableCount;
    ...
    >
      <div className="relative z-10">
        ...
        <motion.p 
          ...
          animate={controls}
        >
          Only {employableCount}% of graduates are job-ready
        </motion.p>
        ...
        >
          <div className="flex items-center">
            ...
            <span className="text-gray-200">{employableCount}% Job-ready</span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded-full bg-gray-600 border border-gray-500 mr-1.5"></div>
            <span className="text-gray-300">{unemployableCount}% Need upskilling</span>
          </div>
        </motion.div>
      </div>
      ...
      <div className="absolute inset-0 overflow-hidden rounded-xl opacity-20 pointer-events-none">
        <div className="absolute -top-20 -right-20 w-40 h-40 bg-green-400 rounded-full filter blur-3xl"></div>
        <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-emerald-500 rounded-full filter blur-3xl"></div>
      </div>
    </div>
  );
};

export default EmployabilityCrisis;
